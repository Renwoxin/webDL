...
from keras.layers import  Conv1D,GlobalMaxPool1D,GlobalMaxPool2D
...
def load_all_files():
    x=[]
    y=[]
    datafile="../data/sms/smsspamcollection/SMSSpamCollection"
...
def do_cnn_wordbag(trainX, testX, trainY, testY):
    ...
    branch1 = Conv1D(128, (3,3), padding='valid', activation='relu',kernel_regularizer=regularizers.l2(0.001))(network)
    branch2 =Conv1D(128, (4,4), padding='valid', activation='relu',kernel_regularizer=regularizers.l2(0.001))(branch1)
    branch3 = Conv1D(128, (5,5), padding='valid', activation='relu',kernel_regularizer=regularizers.l2(0.001))(branch2)
...
def do_cnn_doc2vec_2d(trainX, testX, trainY, testY):
    ...
    # Building convolutional network
    inputs = Input(shape=(max_features, max_document_length, 1), name='input')
    network = Conv2D(16, (3,3), activation='relu')(inputs)
    network = MaxPool2D(pool_size=(2,2))(network)
    network = BatchNormalization()(network)
    network = Conv2D(32,(3,3), activation='relu')(network)
    network = MaxPool2D(pool_size=(2,2))(network)
...
def do_cnn_word2vec_2d(trainX, testX, trainY, testY):
    ...
    # Building convolutional network
    inputs = Input(shape=(max_document_length,max_features,1), name='input')

    network = Conv2D(32,(3,3),activation='relu')(inputs)
    network = MaxPool2D(pool_size=2)(network)
    network = BatchNormalization()(network)
    ...
    model = Model(inputs=inputs,outputs=predictions)
    model.compile(optimizer=adam,loss='categorical_crossentropy')
    model.fit(trainX,trainY,epochs=5,shuffle=True,validation_data=(testX,testY))
...
def do_cnn_word2vec_2d_345(trainX, testX, trainY, testY):
    ...
    # Building convolutional network
    inputs = Input(shape=(max_document_length,max_features,1), name='input')
    network = Embedding(1, 128)(inputs)
    branch1 = Conv2D(128,(3,3),padding='valid',activation='relu')(network)
    branch2 = Conv2D(128,(4,4), padding='valid', activation='relu')(branch1)
    branch3 = Conv2D(128,(5,5),padding='valid',activation='relu')(branch2)
    network = Add(mode='concat', axis=1)(branch1,branch2,branch3)
    network = tf.expand_dims(network, 2)
    ...
    model.fit(trainX, trainY, epochs=5, shuffle=True,
              batch_size=100,validation_data=(testX, testY))
...
def do_cnn_word2vec(trainX, testX, trainY, testY):
    ...
    branch3 = Conv1D(128, (5, 5), padding='valid', activation='relu')(branch1)
    network = Add(mode='concat', axis=1)(branch1,branch2,branch3)
    network = tf.expand_dims(network, 2)
    network = GlobalMaxPool1D()(network)
    network = Dropout(0.8)(network)
...
def do_cnn_doc2vec(trainX, testX, trainY, testY):
    ...
    branch2 = Conv1D(128, 4, padding='valid', activation='relu')(branch1)
    branch3 = Conv1D(128, 5, padding='valid', activation='relu')(branch1)
    network = Add(mode='concat', axis=1)(branch1, branch2, branch3)
    network = tf.expand_dims(network, 2)
    network = GlobalMaxPool1D()(network)
    network = Dropout(0.8)(network)
...
def do_rnn_wordbag(trainX, testX, trainY, testY):
    ...
    model.compile(optimizer=adam, loss='categorical_crossentropy')
    model.fit(trainX, trainY, epochs=5, shuffle=True,
              batch_size=1, validation_data=(testX, testY))
...
def do_rnn_word2vec(trainX, testX, trainY, testY):
    ...
    model.compile(optimizer=adam, loss='categorical_crossentropy')
    model.fit(trainX, trainY, epochs=5, shuffle=True,
              batch_size=10, validation_data=(testX, testY))
