...
def load_all_files():
    ...
    y=[]
    #加载攻击样本
    files=glob.glob("/home/liyulian/websafetyL/data/rootkit/ADFA-LD/ADFA-LD/Attack_Data_Master/*/*.txt")
    for file in files:
        with open(file) as f:
            lines=f.readlines()
        x.append(" ".join(lines))
    ...
    #加载正常样本
    files=glob.glob("/home/liyulian/websafetyL/data/rootkit/ADFA-LD/ADFA-LD/Training_Data_Master/*")
    for file in files:
        with open(file) as f:
            lines=f.readlines()
        x.append(" ".join(lines))
